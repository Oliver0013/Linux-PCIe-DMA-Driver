...
本项目旨在无物理硬件环境下，基于 QEMU 和 Buildroot 从零构建极简的嵌入式 Linux BSP，并针对 QEMU 提供的 edu 教育用设备，开发一个支持多实例、符合标准 VFS 语义的 PCIe 字符设备驱动。

项目核心目标是深入实践 Linux 内核子系统，重点攻克 PCIe 设备枚举与资源映射、现代 MSI 中断处理、一致性/流式DMA以及内核态的细粒度并发控制。

这是一个面向嵌入式 Linux 驱动/内核开发岗位的实战学习项目。通过 QEMU 仿真环境，完整走通从底层系统构建、内核驱动开发到应用层多线程压测的工程闭环。
